...
<main><div class="status"><span class="dot"></span>Modelo cargado y listo para predecir</div><form id="form"><div id="cards" style="display:contents"></div><div class="actions"><button type="reset" class="secondary">Restablecer</button><button id="submit" type="submit">Realizar predicción</button></div><section class="card" id="result"><h2>Resultado del modelo</h2><div id="resultBody"></div></section></form></main>
<script>
const form=document.querySelector('#form'),cards=document.querySelector('#cards'),result=document.querySelector('#result');
function field(name,s){const label=document.createElement('label');label.textContent=s.label;let el;if(s.type==='number'){el=document.createElement('input');el.type='number';el.min=s.min;el.max=s.max;el.step=1;el.value=s.default}else{el=document.createElement('select');s.options.forEach(o=>{const op=document.createElement('option');op.value=o.value;op.textContent=o.label;op.selected=o.value==s.default;el.append(op)})}el.name=name;el.required=true;label.append(el);return label}
...
def crear_configuracion():
    ...
    for col in COLUMNAS:
        if col == "edad":
            ...
            fields[col] = {"label": ETIQUETAS.get(col, col), "type": "select", "options": [{"value": v, "label": v} for v in valores], "default": str(moda(TRAIN[col].fillna("DESCONOCIDO")))}
