...
def main():
    ...
    rep.p(f"Python {sys.version.split()[0]} | scikit-learn {sklearn.__version__} | "
          f"semilla {SEMILLA}")
    rep.p("Libreria: sklearn.neural_network.MLPClassifier")
    rep.p("(no TensorFlow/Keras: sin dependencias nuevas y misma API que los")
    rep.p(" modelos 1-3. Ver docstring para la justificacion completa.)")
    ...
    if ruta_cb.exists():
        rep.p("  (CatBoost usa otra codificacion de entrada; su F1 val es 0.6392")
        rep.p("   segun su propio informe, no se recalcula aqui.)")
